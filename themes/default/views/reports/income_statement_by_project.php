                ...
                <div class="table-scroll">
					<table class="table table-bordered">
						<thead>
							...
							?>
								<th class="text-center" colspan="12" style="border-right: 2px solid #000;"><?php echo $years_arr[$m_i]; ?></th>
							...
							for($m_i = 0; $m_i < sizeOf($years_arr); $m_i++){ ?>
								...
								<th style="text-align:left;border-right:2px solid black !important"><div class="fix-text-moth"><?= lang("Dec"); ?></div></th>
						...
						<tbody>
								...
								foreach ($new_billers as $biller1) {
									...
									for($m_i = 0; $m_i < sizeOf($years_arr); $m_i++){
										...
										foreach($monthlyIncomesYear->result() as $project){
											...
											<td style="border-right:2px solid black !important"><div class="fix-text-moth"><?=number_format(abs($project->dec), 2)?></div></td>
									...
									<td><div class="fix-text-moth"><b><?= $this->erp->formatDecimal(abs($total_per))?></b></div></td>
											<td><div class="fix-text-moth"><b><?=$this->erp->formatDecimal($project->total_amount - (abs($total_per) + $project->begining_balance))?></b></div></td>
									
									</tr>
									<?php
									$total_m += abs($total_per);
									$total_balance_m += $project->total_amount - (abs($total_per) + $project->begining_balance);
									$sum_f_total_balance += $total_balance_m;
									$total_per = 0;
								}
								?>
							<tr>
							...
							for($m_i = 0; $m_i < sizeOf($years_arr); $m_i++){ 
								...
								$sum_f_total += $jan_a + $feb_a + $mar_a + $apr_a + $may_a + $jun_a + $jul_a + $aug_a + $sep_a + $oct_a + $nov_a + $dec_a;
								?>
								...
								<th style="border-right:2px solid black !important"><div class="fix-text-moth"><?=number_format($dec_a,2)?></div></th>
								
							<?php 
								}
								?>
								<th><div class="fix-text"><?=number_format($sum_f_total,2)?></div></th>
								<th><div class="fix-text"><?=number_format($sum_amount - ($sum_f_total + $f_begin),2)?></div></th>
							</tr>
						</tbody>
					</table>
                </div>
