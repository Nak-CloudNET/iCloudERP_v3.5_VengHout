...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-star"></i><?=lang('project_plan_report');?>
        </h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                ...
                <div class="table-responsive">
                    <table id="POData" cellpadding="0" cellspacing="0" border="0" class="table table-condensed table-bordered table-hover table-striped">
						...
						<tr class="active">
							...
							<th class="text-center"><?php echo $this->lang->line("project_plan"); ?></th>
							<th class="text-center"><?php echo $this->lang->line("using"); ?></th>
                        ...
                        <?php 
                            ...
                            foreach($pplans as $pplan) {
                                $items = $this->reports_model->getProjectPlanAndUsing($pplan->id, $pplan->address_id);
                                if(is_array($items)){
                                    $am = 0;
                            ?>
                            <tr class="success">
                                <th class="th_parent" colspan="4">
                                    <?= lang("project_plan")?> <i class="fa fa-angle-double-right" aria-hidden="true"></i> <?= $pplan->plan ?> |
                                    <?= lang("address")?> <i class="fa fa-angle-double-right" aria-hidden="true"></i> <?= $pplan->address ?>
                                </th>
                            </tr>
                            ...
                            ?>
                                <tr>
                                    <td style="padding-left: 50px !important;"><?= $row->product_name; ?></td>
                                    <td class="text-right"><?= $this->erp->formatQuantity($row->project_plan) ?></td>
                                    <td class="text-right"><?= $this->erp->formatQuantity($row->using_stock) ?></td>
                                    <td class="text-right"><?= $this->erp->formatMoney($row->balance) ?></td>
                                </tr>
                            <?php
                                $total_sale += $row->balance;
                                    }
                            ?>
                            <tr>
