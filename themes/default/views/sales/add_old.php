<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        <?php if($quote_id) { ?>
        __setItem('sldate', '<?= $this->erp->hrld($quote->date) ?>');
        __setItem('slcustomer', '<?= $quote->customer_id ?>');
        __setItem('slbiller', '<?= $quote->biller_id ?>');
        __setItem('slwarehouse', '<?= $quote->warehouse_id ?>');
        __setItem('slnote', '<?= str_replace(array("\r", "\n"), "", $this->erp->decode_html($quote->note)); ?>');
        __setItem('sldiscount', '<?= $quote->order_discount_id ?>');
        __setItem('sltax2', '<?= $quote->order_tax_id ?>');
        __setItem('slshipping', '<?= $quote->shipping ?>');
        __setItem('slitems', JSON.stringify(<?= $quote_items; ?>));
        <?php } ?>
											...
											<div class="row">
												...
												<div class="col-md-3">
													<div class="form-group" id="print_" style="display:none">
														...
														<!--<button id="excel_depre" type="button" class="btn btn-success col-md-12"><i class="fa fa-file-excel-o"></i>&nbsp;<?= lang("Export to Excel") ?></button>-->
														<div style="clear:both; height:15px;"></div>
														<!--<span class="pull-right col-xs-12">
															<a href="<?=base_url()?>pos/depreciation_report/<?=$amount?>" target="_blank" id="depreciation_print" class="btn btn-block btn-primary" ><i class="fa fa-print"></i> <?= lang("print"); ?></a>
														</span>-->
													</div>
												 </div>
											</div>
	...
	function billerChange(){
			...
			$.ajax({
				...
				success: function(result){
					...
					$("#slwarehouse option:first").attr('selected','selected');
				}
			});	
		}	
		...
		function PopupPayments() {
			...
			mywindow.document.write("<center><h3 style='font-family:Verdana,Geneva,sans-serif;'>Loan Amortization Schedule</h3></center><br/>");
			mywindow.document.write('<table class="table-condensed" style="width:95%; font-family:Verdana,Geneva,sans-serif; font-size:12px; padding-bottom:10px;">'+
										'<tr>'+
			...
			mywindow.document.write(			'<tr>'+
											...
											var loan_amount = total_amt - down_pay;
			...
			mywindow.document.write('<table border="2px" class="table table-bordered table-condensed table_shape" style="width:95%; font-family:Verdana,Geneva,sans-serif; font-size:12px; border-collapse:collapse;">'+
										 '<thead>'+
											  '<tr>'+
												'<th width="5%" class="td_bor_style"><?= lang('Period') ?></th>'
									);
											if(type == 2){
			...
			mywindow.document.write(			'<th width="10%" class="td_bor_style"><?= lang('Interest') ?></th>'+
												...
												'<th width="15%" class="td_bor_style"><?= lang('Payment') ?></th>'
									);
											}
			mywindow.document.write(			'<th width="10%" class="td_bor_style"><?= lang('Balance') ?></th>'+
												'<th width="15%" class="td_bor_style"><?= lang('Due Month') ?></th>'+
