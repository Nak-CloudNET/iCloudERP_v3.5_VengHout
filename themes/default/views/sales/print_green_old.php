...
<div style="margin-top:20px;width:30%;position:absolute;text-align:center;">
	 <img src="'. base_url() . 'assets/uploads/logos/'.$logo.'" alt="company_logo" width="200"/> 
</div>
<div style="width:300px;margin:0 auto;">
	<div style="text-align:center;font-weight:bold;font-size:16px;margin-top: 0;">'.$biller_company.'</div>
...
<div>
	<p style="font-size:16px;font-family:Khmer OS MUOL;text-align:center;font-weight:bold;"">វិក័យប័ត្រ</p>
</div>
	<div style="float: left" class="inv-left">
		...
		<p>
			...
			<div style="margin-top:-15px;float:left;width:200px;font-size:13px;font-family:Khmer OS"><b><p style="line-height:20px;">'.$address.' </p></b></div>
			...
			';
				   ...
				   foreach ($rows as $row):
					...
					if($pos->show_product_code == 0) {
						$product_name = $row->product_name . ($row->variant ? ' (' . $row->variant . ')' : '');
					}else{
						$product_name = $row->product_name . " (" . $row->product_code . ")" . ($row->variant ? ' (' . $row->variant . ')' : '');
					}
					*/
					
					$qty=$row->subtotal!=0?$this->erp->formatMoney($row->unit_price):$free;
					$amount=$row->subtotal!=0?$this->erp->formatMoney($row->subtotal):$free;
					if($i==18||$i==36||$i==52 || $i==70){
						$tablebody[$n_page].='<tr  style="border-bottom:solid 1px black;">
						<td>'.$i.'</td>
						<td>'.$product_name.'</td>
						<td>'.number_format($row->quantity, 0).''.$unit.'</td>
						<td>'.$qty.'</td>
						<td>'.$discount.'</td>
						<td>'.$amount.'</td>
						</tr>';
...
for($j=1;$j<=$n_page;$j++){
	echo '<div>'.$head.'</div><div>'.$table.''.$tablebody[$j];
		...
		echo $tablebtm.''.$tableend.'</div>'.$btm.'<div class="page-break"></div>';
			...
			echo $tableend.'</div>'.$btmnon.'<div class="page-break"></div>';
		}
}
?>
</body>
