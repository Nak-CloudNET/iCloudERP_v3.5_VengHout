...
<div id="wrapper">
    ...
    <div id="receipt-data">
        ...
        <div class="text-center">
			...
			<div class="col-xs-4">
			<img src="<?= base_url() . 'assets/uploads/logos/' . $biller->logo; ?>" alt="<?= $biller->company; ?>" style='float:left'> 
			</div>
			<div class="col-xs-8">
			...
			echo '<div style="margin-top:5px;margin-bottom:5px;">';
				echo lang("invoice") . " : " . $inv->reference_no . "<br>";
            ...
            <table class="table table-striped table-condensed receipt">
                ...
                <tr>
					<th></th>
					<th><!--<?= lang("exchange_rate"); ?> : <?php echo $exchange_rate_kh_c->rate?number_format($exchange_rate_kh_c->rate):0 ?> ៛ | <?= lang("qty"); ?>= (<?=$this->erp->formatQuantity($total_quantity)?>)--></th>
				...
				if($pos_paid >= $inv->grand_total){
					...
					<?php if((($pos_paid+$amount_kh_to_us) - $inv->grand_total) != 0 && ($this->erp->formatMoney((($pos_paid+$amount_kh_to_us) - $inv->grand_total)*$exchange_rate->rate )) != 0){ ?>
					<tr>
                    ...
                    <tr>
						...
						<th style="border-top:2px dotted #000" class="text-right"><?= number_format(round((($pos_paid+$amount_kh_to_us) - $inv->grand_total)*$exchange_rate->rate), -3) . '៛' ; ?></th>
                    </tr>
					-->
				<!--	<tr>
                        <th style="border-top:2px dotted #ddd;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right">Due Amount(BAHT)</th>
                        <th style="border-top:2px dotted #ddd" class="text-right"><?= $this->erp->formatMoney((($pos_paid+$amount_kh_to_us) - $inv->grand_total)*$exchange_rate_th->rate ); ?></th>
                    </tr> -->
					<?php
					}
				}
				...
				<!--
                    <tr>
                        <th style="border-top:2px dotted #000;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right"><?= lang("due_amount_kh"); ?></th>
                        <th style="border-top:2px dotted #000" class="text-right"><?= number_format((($pos_paid+$amount_kh_to_us) - $inv->grand_total)*$exchange_rate->rate) . ' ៛' ; ?></th>
                    </tr>
					-->
				<!--	<tr>
                        <th style="border-top:2px dotted #ddd;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right">Due Amount(BAHT)</th>
...
<?php } ?>
	...
	<span class="pull-right col-xs-12">
        <a href="<?=base_url()?>sales/invoice_landscap_a5/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("print_invoice"); ?></a>
    </span>
	<span class="pull-right col-xs-12">
