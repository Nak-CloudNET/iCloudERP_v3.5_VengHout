...
class Payments extends MY_Controller
{
    ...
    function paypalipn()
    {
        ...
        $this->erp->log_payment('Paypal IPN called');
        ...
        if (!$fp) {
            ...
            $this->erp->log_payment('Paypal Payment Failed (IPN HTTP ERROR)', $errstr);
            ...
            while (!feof($fp)) {
                ...
                if (stripos($res, "VERIFIED") !== false) {
                    $this->erp->log_payment('Paypal IPN - VERIFIED');
                    ...
                    if (($_POST['payment_status'] == 'Completed' || $_POST['payment_status'] == 'Processed' || $_POST['payment_status'] == 'Pending') &&
                        ...
                        if ($inv = $this->sales_model->getInvoiceByID($invoice_no)) {
                            ...
                            if ($this->sales_model->addPayment($payment)) {
                                ...
                                $this->erp->send_email($paypal->account_email, 'Payment has been made via Paypal', $message);
                                $this->erp->log_payment('Payment has been made for Sale Reference #' . $_POST['item_name'] . ' via Paypal (' . $_POST['txn_id'] . ').', print_r($_POST, ture));
                            }
                        }
                    } else {

                        $this->erp->log_payment('Payment failed for Sale Reference #' . $reference . ' via Paypal (' . $_POST['txn_id'] . ').', print_r($_POST, ture));

                    }
                } else if (stripos($res, "INVALID") !== false) {
                    $this->erp->log_payment('INVALID response from Paypal. Payment failed via Paypal.', print_r($_POST, ture));
                }
            }
            fclose($fp);
        }
    ...
    function skrillipn()
    {
        ...
        $this->erp->log_payment('Skrill IPN called');
        ...
        if (strtoupper(md5($concatFields)) == $_POST['md5sig'] && $_POST['status'] == 2 && $_POST['pay_to_email'] == $skrill->account_email) {
            ...
            if ($inv = $this->sales_model->getInvoiceByID($invoice_no)) {
                ...
                if ($this->sales_model->addPayment($payment)) {
                    ...
                    $msg = file_get_contents('./themes/' . $theme . '/views/email_templates/payment.html');
                    $message = $this->parser->parse_string($msg, $parse_data);
                    $this->erp->send_email($skrill->account_email, 'Payment has been made via Skrill', $message);
                    $this->erp->log_payment('Payment has been made for Sale Reference #' . $_POST['item_name'] . ' via Skrill (' . $_POST['mb_transaction_id'] . ').', print_r($_POST, ture));
                }
            }
        } else {
            $this->erp->log_payment('Payment failed for via Skrill.', print_r($_POST, ture));
            exit;
        }
