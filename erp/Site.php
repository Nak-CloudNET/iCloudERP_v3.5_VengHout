...
class Site extends CI_Model
{
	...
	*/
	...
	public function core_js() {
        return '<script type="text/javascript">' . file_get_contents($this->data['assets'] . 'js/corel.js') . '</script>';
    }
    ...
    public function getReference($field) {
        ...
        if ($q->num_rows() > 0) {
            ...
            if ($this->Settings->reference_format == 1) {
                ...
                $ref_no .= date('ym') . "-" . date('m') . "-" . sprintf("%05s", $ref->{$field});
	...
	public function calculateAVCosts($product_id, $warehouse_id, $net_unit_price, $unit_price, $quantity, $product_name, $option_id, $item_quantity, $shipping) {
        ...
        if ($pis = $this->getPurchasedItems($product_id, $warehouse_id, $option_id)) {
            ...
            foreach ($pis as $pi) {
				...
				if($pi->item_tax == 0){
					$ship = $this->getProjectpingItems($pi->purchase_id);
					//foreach($ship as $shi){
						$total_unit_cost += ($pi->real_unit_cost*$pi->quantity_balance);
						$total_unit_costs += $pi->quantity_balance;
						//$total_shoping += $shi->shipping;
					//}
				}else{
					$total_unit_cost += ($pi->unit_cost ? $pi->unit_cost : $pi->net_unit_cost + ($pi->item_tax / $pi->quantity_balance));
				}
            }
			$new_unit_cost = ($unit_price*$item_quantity)+$shipping;
			$final_cost    = $new_unit_cost + $total_unit_cost;
			$new_unit_costs= $total_unit_costs + $item_quantity;
            $as = sizeof($pis);
            $avg_net_unit_cost = $total_net_unit_cost / $as;
			$avg_unit_cost = $final_cost/$new_unit_costs;
			//$avg_unit_cost = $total_unit_cost/$total_unit_costs;
    ...
    public function calculateCost($product_id, $warehouse_id, $net_unit_price, $unit_price, $quantity, $product_name, $option_id, $item_quantity) {
        ...
        $balance_qty = $quantity;
        foreach ($pis as $pi) {
            if (!empty($pi) && (($balance_qty <= $quantity && $quantity > 0) || $quantity < 0 ) ) {
    ...
    public function getPurchasedItems($product_id, $warehouse_id, $option_id = NULL) {
        $orderby = ($this->Settings->accounting_method == 1) ? 'asc' : 'desc';
        $this->db->select('id, quantity, quantity_balance, net_unit_cost, unit_cost, item_tax, real_unit_cost');
    ...
    public function syncQuantity($sale_id = NULL, $purchase_id = NULL, $oitems = NULL, $product_id = NULL) {
        if ($sale_id) {
			...
			//$this->erp->print_arrays($purchase_items);
